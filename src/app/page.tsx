import { LinkWidget } from '@/app/ui/widgets/LinkWidget';
import { ClockWidget } from './ui/widgets/ClockWidget';
import { TodoWidget } from './ui/widgets/TodoWidget/TodoWidget';

export default function Home() {
  return (
    <main className='flex items-center justify-center min-h-screen min-w-full overflow-clip p-8 md:p-28'>
      <div className='grid grid-flow-dense grid-cols-2 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 2xl:grid-cols-12 gap-16'>
        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />

        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
          tiles={[2, 2]}
        />

        <ClockWidget tiles={[3, 3]} />

        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />

        <TodoWidget />

        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />
        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />

        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />
        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />

        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />
        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />

        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />
        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />
        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />

        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
          tiles={[3, 1]}
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
          tiles={[3, 2]}
        />

        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />
        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />

        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />
        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />

        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />
        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />

        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />
        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />
        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />

        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
        <LinkWidget
          title='instagram'
          image='https://cdn.pixabay.com/photo/2021/06/15/12/14/instagram-6338393_1280.png'
          url='https://instagram.com'
        />

        <LinkWidget
          title='gmail'
          image='https://images.indianexpress.com/2022/01/Gmail.jpg'
          url='https://mail.google.com'
        />
        <LinkWidget
          title='YouTube'
          image='https://static-00.iconduck.com/assets.00/youtube-icon-2048x2048-wiwalbpx.png'
          url='https://youtube.com'
        />

        <LinkWidget
          title='netflix'
          image='https://static-00.iconduck.com/assets.00/netflix-icon-icon-2048x2048-yj41gpvr.png'
          url='https://netflix.com'
        />
      </div>
    </main>
  );
}
